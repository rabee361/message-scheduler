...
export const handleValidationErrors = (
  ...
  if (!errors.isEmpty()) {
    const validationErrors = errors.array().map((error) => {
      // Type guard: check that param, value, location exist
      if ('param' in error && 'value' in error && 'location' in error) {
        return {
          field: error.param,
          message: error.msg,
          value: error.value,
        ...
        // fallback for AlternativeValidationError or unknown shapes
        return {
          field: 'unknown',
          message: error.msg,
          value: null,
