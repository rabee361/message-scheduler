...
router.post('/:sessionId/send', [
  ...
  body('to').notEmpty().trim().custom((value) => {
    ...
    // Check if it contains @ which is required for a valid JID
    if (!value.includes('@')) {
      throw new Error('Invalid JID format: missing @');
    }
    
    // Check if it has valid server part
    const parts = value.split('@');
    if (parts.length !== 2 || !parts[1]) {
      throw new Error('Invalid JID format: invalid server part');
    }
    
    const serverPart = parts[1];
    const validServers = ['s.whatsapp.net', 'c.us', 'g.us', 'broadcast', 'status', 'newsletter'];
    if (!validServers.includes(serverPart)) {
      // Also check if it ends with one of the valid servers
      const validEndings = ['.whatsapp.net', '.us'];
      if (!validEndings.some(ending => serverPart.endsWith(ending))) {
        throw new Error('Invalid JID format: invalid server');
      }
    }
  ...
  try {
    if (sessionId) {
      ...
      const sessionExistsInSystem = await dbService.getSession(sessionId)
